...
then
	...
	git_info() {
		#git_ps1=$(__git_ps1)
		# Avec '%s' pour ne pas avoir les parenthèses
		git_ps1=$(__git_ps1 '%s')
		if [[ -n "$git_ps1" ]]
		then
			if [[ "$git_ps1" =~ GIT_DIR ]]
	...
	# Exportation des fonctions nécessaire au prompt, sinon, le terminal
	# ne les trouve pas pour crééer le prompt quand on est dans un virtenv.
